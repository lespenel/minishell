...
/*   Updated: 2024/03/24 23:51:08 by lespenel         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	compare_files(t_wildcard *wild, t_lexer *fnames, t_lexer *pattern, char *to_cmp);
...
int	for_each_file(t_wildcard *wild, t_pattern *pattern, t_lexer *filenames)
{
	...
	while (i < old_size)
	{
		...
		if (compare_files(wild, &new_dir_lst, &pattern->pattern,
			tok->content) == -1)
			return (-1);
...
int	get_matching_filenames(t_wildcard *wildcard, t_lexer *filenames)
{
	...
	{
		if (compare_files(wildcard, filenames,
			&pattern->pattern, NULL) == -1)
			return (-1);
	}
	else
	{
		if (for_each_file(wildcard, pattern, filenames) == -1)
			return (-1);
	}
	return (0);
}
...
int	compare_files(t_wildcard *wild, t_lexer *fnames, t_lexer *pattern, char *to_cmp)
{
	...
	j = 0;
	if (get_files_ls(wild, &new_filenames, to_cmp) == -1)
	{
